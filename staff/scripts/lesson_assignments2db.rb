...
["ΒΗ", "Εισαγωγή στα Υπολογιστικά Συστήματα και στα Δίκτυα Επικοινωνιών", 
 ["ΠΕ12.06","ΠΕ12.10",
  "ΠΕ17.04","ΠΕ17.08","ΠΕ17.09",
  "ΤΕ01.07"],
  ["ΠΕ12.05",
...
lesson_assignments.each do |la|
  puts ">>>>"
  if la.length>=1+1+1+3
    puts "#{la[0]} #{la[1]} #{la[2]}"
    puts la[3].join(",") if la.length>3
    puts la[4].join(",") if la.length>4
    puts la[5].join(",") if la.length>5
  else
    puts "#{la[0]} #{la[1]}"
    puts la[2].join(",") if la.length>2
    puts la[3].join(",") if la.length>3
    puts la[4].join(",") if la.length>4
    puts la[5].join(",") if la.length>5
  end
